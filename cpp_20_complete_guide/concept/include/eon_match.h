...
public:
    ...
    {
        std::cout << "构造 MatchHelper" << std::endl;
    }
...
#define UN_OP_FOR_UNARY(op)                                              \
    template <typename T,                                                \
              std::enable_if_t<isUnaryOrWildcardV<T>, bool> = true>      \
    constexpr auto operator op(T const &t)                               \
    {                                                                    \
        return unary(                                                    \
            [&](auto &&arg) constexpr { return op evaluate_(t, arg); }); \
    }
...
{
public:
    constexpr explicit PatternHelper(Pattern const &pattern) : mPattern{pattern}
    {
    }
                                           ...
                                           int32_t depth, ContextT &context)
    {
        ...
        return std::apply(
                   [&value, depth, &context](auto const &...patterns) {
                       return (
                           matchPattern(value, patterns, depth + 1, context) ||
                           ...);
                   },
                   take<patSize - 1>(orPat.patterns())) ||
               matchPattern(std::forward<Value>(value),
                            get<patSize - 1>(orPat.patterns()), depth + 1,
                            context);
    }
...
{
public:
    constexpr explicit And(Patterns const &...patterns) : mPatterns{patterns...}
    {
    }
    constexpr auto const &patterns() const { return mPatterns; }
...
public:
                                           ...
                                           int32_t depth, ContextT &context)
    {
                          ...
                          andPat.patterns()))>>::nbIdV == 0)
        {
            return exceptLast &&
                   matchPattern(std::forward<Value>(value),
                                get<patSize - 1>(andPat.patterns()), depth + 1,
                                context);
        }
        else
        {
            return exceptLast &&
                   matchPattern(value, get<patSize - 1>(andPat.patterns()),
                                depth + 1, context);
        }
    }
...
public:
    ...
    constexpr static void processIdImpl(Not<Pattern> const &notPat,
                                        int32_t depth, IdProcess idProcess)
    {
...
struct StorePointer<Type, Value,
                    std::void_t<decltype(std::declval<ValueVariant<Type> &>() =
                                             &std::declval<Value>())>>
    : std::is_reference<Value> // need to double check this condition. to loosen
                               // it.
{
};
...
public:
    ...
    {
        return std::visit(
            overload([](Type const &) { return true; },
...
public:
    ...
    constexpr static auto matchPatternImpl(
        Value &&value, OooBinder<Pattern> const &oooBinderPat, int32_t depth,
        ContextT &context)
    {
...
public:
    ...
    public:
        using type = decltype(std::tuple_cat(
            std::declval<typename PatternTraits<
                Patterns>::template AppResultTuple<Values>>()...));
    };

    template <typename... Values>
    class AppResultForTupleHelper<1, std::tuple<Values...>>
    {
        constexpr static auto idxOoo =
            findOooIdx<typename Ds<Patterns...>::Type>();
        ...
        using Vs1 =
            SubTypesT<vs1Start, sizeof...(Values), std::tuple<Values...>>;
    ...
    class AppResultHelper<
        RangeType,
        std::enable_if_t<!isTupleLikeV<RangeType> && isRangeV<RangeType>>>
    {
    public:
    ...
    constexpr static auto matchPatternImpl(
        ValueTuple &&valueTuple, Ds<Patterns...> const &dsPat, int32_t depth,
        ContextT &context) -> std::enable_if_t<isTupleLikeV<ValueTuple>, bool>
    {
        if constexpr (nbOooOrBinder == 0)
        {
            return std::apply(
                [&valueTuple, depth, &context](auto const &...patterns) {
                    return apply_(
                        [depth, &context,
                         &patterns...](auto &&...values) constexpr {
        ...
        {
            constexpr auto idxOoo =
                findOooIdx<typename Ds<Patterns...>::Type>();
            constexpr auto isBinder = isOooBinderV<
                std::tuple_element_t<idxOoo, std::tuple<Patterns...>>>;
            ...
            if constexpr (isArray)
            {
                ...
                {
                    auto const rangeSize =
                        static_cast<long>(valLen - (patLen - 1));
                    context.emplace_back(makeSubrange(
                        &valueTuple[idxOoo], &valueTuple[idxOoo] + rangeSize));
                    using type = decltype(makeSubrange(
                        &valueTuple[idxOoo], &valueTuple[idxOoo] + rangeSize));
                    result = result &&
                             matchPattern(std::get<type>(context.back()),
            ...
            return result &&
                   matchPatternMultiple<valLen - patLen + idxOoo + 1,
                                        idxOoo + 1, patLen - idxOoo - 1>(
                       std::forward<ValueTuple>(valueTuple), dsPat.patterns(),
                       depth, context);
        }
    }
                            ...
                            bool>
    {
        ...
        if constexpr (nbOooOrBinder == 0)
        {
            ...
            return matchPatternRange<0, nbPat>(
        ...
        else if constexpr (nbOooOrBinder == 1)
        {
            ...
            if constexpr (isBinder)
            {
                ...
                result =
                    result && matchPattern(std::get<type>(context.back()),
                                           std::get<idxOoo>(dsPat.patterns()),
                                           depth, context);
            }
            ...
            return result &&
                   matchPatternRange<idxOoo + 1, patLen - idxOoo - 1>(
...
public:
    ...
    constexpr static void processIdImpl(
        PostCheck<Pattern, Pred> const &postCheck, int32_t depth,
        IdProcess idProcess)
    {
...
public:
    ...
    // cast to base class
    template <typename D, typename std::enable_if<!viaGetIfV<T, D> &&
                                                  std::is_base_of_v<T, D>>::type
                              * = nullptr>
    ...
    // No way to handle rvalue to save copy in this class. Need to define some
    // in another way to handle this. cast to base class
    template <typename D, typename std::enable_if<!viaGetIfV<T, D> &&
                                                  std::is_base_of_v<T, D>>::type
                              * = nullptr>
    ...
    template <typename B, typename std::enable_if<!viaGetIfV<T, B> &&
                                                  std::is_base_of_v<B, T>>::type
                              * = nullptr>
