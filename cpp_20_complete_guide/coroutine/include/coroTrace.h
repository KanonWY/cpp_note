...
public:
    ...
    int getYieldValue() const {
        return hdl.promise().yield_value_store;
    }
...
struct TraceAwaiter {
     ...
     *        根据返回值可以执行对执行流进行切换
     *        1、如果返回值类型为 void， 表示执行完该函数之后，无条件的挂起，然后返回给协程的调用者和恢复者
     *
     *        2、如果返回值为 bool 值， true： 挂起当前协程并返回给当前协程的调用者和恢复者（注意当前协程已经为挂起状态了）
     *                                 false： 直接恢复当前协程（注意当前协程已经为挂起状态了)
